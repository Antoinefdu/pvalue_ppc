...
'''Please run "start.py" and "fill_tables.py" BEFORE running this script. 
    Steps : 
        1 - Run the script
        2 - Choose your alpha level, or leave blank, then press "Enter"
        3 - Select a database
        4 - That's it
'''
...
if __name__ == '__main__':
    ...
    keep_asking = True
    while keep_asking:
        alpha = input('Please enter a significance level that is between 0 and 0.5 (excluded), or press "Enter" to keep the default value (0.05)\n') \
                or 0.05
        try:
            alpha = float(alpha)
    ...
    with sql.connect(db_choice) as conn:
        ...
        for ad_group in c.fetchall():
            ...
            df = functions.fetch_data_with_condition2(c, table_dict['Ads'],
                                                      {'AD_GROUP': ag_name, 'CAMPAIGN': campaign_name}, ('ID', 'IMPRESSIONS', 'CLICKS'))
